...
func (n *TCPNat) checkTimeout() {
	...
	for natPort, session := range n.portMap {
		...
		if session.Closed {
			timedOut = !session.ClosingUntil.IsZero() && !now.Before(session.ClosingUntil)
		}
		session.Unlock()
		if timedOut {
	...
	for _, e := range expired {
		if n.portMap[e.port] != e.session {
			continue
		}
		e.session.Lock()
		timedOut := now.Sub(e.session.LastActive) >= n.timeout
		if e.session.Closed {
			timedOut = !e.session.ClosingUntil.IsZero() && !now.Before(e.session.ClosingUntil)
		}
		if timedOut {
			key := tcpNatKey{Source: e.session.Source, Destination: e.session.Destination}
			if mappedPort, loaded := n.addrMap[key]; loaded && mappedPort == e.port {
